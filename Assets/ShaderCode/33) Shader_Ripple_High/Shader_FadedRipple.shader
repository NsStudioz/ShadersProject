Shader "Unlit/Vertex_Centered"
{
    Properties // input data
    {
        ...
        _WaveAmp("Wave Amplitude", Range(0,10)) = 0       // define property.
    }
        SubShader
    {
        Tags 
        ...
        Pass
        {
            ...
            {
                float2 uvsCentered = uv * 2 - 1;  // centering UV coords.
                float radialDistance = length(uvsCentered); // radial circle.
                float wave = cos((radialDistance - _Time.y * 0.1) * TAU * 1) * 0.5 + 0.5; // using time to move the circle from the center.

                wave *= 1 - radialDistance; // invert the position of the fade using 1 -. Now fades at the end.
                return wave; // Output.
            }
