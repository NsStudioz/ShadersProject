Shader "Unlit/UV_Zigzag_Pattern_Animated"
{
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Interpolators i) : SV_Target
            {
                ...
                float xOffset = cos(i.uv.y * TAU * 8) * 0.01; // reduce last value in the calculations to soften the ZigZag pattern (0.01).
                float t = cos( (i.uv.x + xOffset + _Time.y * 0.1) * TAU * 5) * 0.5 + 0.5; // still a repeating pattern but the range has been shifted when multiply by 0.5 & add 0.5.
                                                                                    // added _Time.y. This animates the pattern based on time.
